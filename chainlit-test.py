...
metadata = MetaData()
target_table = Table('threads', metadata, autoload_with=engine)
Session = sessionmaker(bind=engine)
session = Session()

def cleanup_threads():
    try:
        delete_stmt = target_table.delete().where(
            and_(
                target_table.c.name.is_(None),
                target_table.c.userId.is_(None),
                target_table.c.userIdentifier.is_(None)
            )
        )
        result = session.execute(delete_stmt)
        session.commit()
        print(f"Cleanup complete. Rows removed: {result.rowcount}")
