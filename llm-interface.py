...
async def search(state: AgentState) -> Command[Literal['router']]:
    tavily_tool = TavilySearch(search_depth='advanced',max_results=5,time_range='year')
    question = state["messages"][-1].content
    research_agent = create_agent(
        llm,  
        tools=[tavily_tool],  
        system_prompt= "You are an Information Specialist with expertise in comprehensive research. Your responsibilities include:\n\n"
    ...
    search_result = await research_agent.ainvoke({"messages": [{"role": "user", "content": f"Using the tool, find information on: {question}"}]})
    return Command(goto='router', update={'messages': [AIMessage(content=search_result["messages"][-1].content)]})
...
graph.add_node('search_tool', search)
